...
class ImapFullScenario extends Simulation {
  ...
  val userCount: Int = feeder.readRecords.length
  ...
  val scn = scenario("Imap").feed(feeder.circular)
    .pause(1 second)
  ...
  setUp(scn.
      inject(
          ...
          rampUsers(userCountWith50PercentsOfThemAlsoReadingTheirEmailOnAnotherDevice) during (10 minutes)))
