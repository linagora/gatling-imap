...
object UnselectAction {
  def props(imapContext: ImapActionContext, requestName: String, checks: Seq[ImapCheck]) =
    Props(new UnselectAction(imapContext, requestName, checks))
}

class UnselectAction(val imapContext: ImapActionContext,
  ...
  override protected def executeOrFail(session: Session): Validation[_] = {
    Validation.unit
      .map(_ => {
        ...
        sessions.tell(Command.Unselect(UserId(id)), handler)
      })
  }
}
