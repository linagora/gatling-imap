...
object UidMoveAction {
  def props(imapContext: ImapActionContext, requestName: String, checks: Seq[ImapCheck], sequence: Expression[MessageRanges], mailbox: Expression[String]) =
    Props(new UidMoveAction(imapContext, requestName, checks, sequence, mailbox))
}

class UidMoveAction(val imapContext: ImapActionContext,
  ...
  override protected def executeOrFail(session: Session): Validation[_] = {
    for {
      ...
      sessions.tell(Command.UidMove(UserId(id), sequence, mailbox), handler)
    }
  }
}