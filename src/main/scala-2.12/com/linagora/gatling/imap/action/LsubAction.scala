...
object LsubAction {
  def props(imapContext: ImapActionContext, requestname: String, checks: Seq[ImapCheck], reference: Expression[String], name: Expression[String]) =
    Props(new LsubAction(imapContext, requestname, checks, reference, name))
}

class LsubAction(val imapContext: ImapActionContext, val requestName: String, override val checks: Seq[ImapCheck], reference: Expression[String], name: Expression[String]) extends ValidatedActionActor with ImapActionActor {
  ...
  override protected def executeOrFail(session: Session): Validation[_] = {
    for {
      ...
      sessions.tell(Command.Lsub(UserId(id), reference, name), handler)
    }
  }
}