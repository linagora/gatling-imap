...
object ImapUidMoveMessageScenario {
  val numberOfMailInInbox = 5
  ...
  def apply(feeder: FeederBuilder): ScenarioBuilder =
    scenario("Imap")
      ...
      .exec(imap("uidMove").uidMoveMessage(MessageRanges(Range(1, numberOfMailInInbox)), "Another INBOX").check(ok))
