...
import com.linagora.gatling.imap.protocol.command.GetAclCommand
import io.gatling.commons.validation.Validation
import io.gatling.core.session._

import scala.collection.immutable.Seq

class GetAclAction(imapContext: ImapActionContext,
                   requestName: String,
  ...
  override def sendRequest(session: Session): Validation[Unit] = {
    ...
    for {
      ...
      val future = s.execute(new GetAclCommand(mailbox))
