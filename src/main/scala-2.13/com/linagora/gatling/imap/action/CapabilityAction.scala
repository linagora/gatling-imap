...
object CapabilityAction {
  def props(imapContext: ImapActionContext, requestName: String, checks: Seq[ImapCheck]) =
    Props(new CapabilityAction(imapContext, requestName, checks))
}

class CapabilityAction(val imapContext: ImapActionContext,
  ...
  override protected def executeOrFail(session: Session): Validation[_] = {
    Validation.unit
      .map(_ => {
        ...
        sessions.tell(Command.Capability(UserId(id)), handler)
      })
  }
}
